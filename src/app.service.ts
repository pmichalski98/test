  ...
  async handleMarkdown(question: string) {
    ...
    const completion = await openai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: `Transform given text in markdown format into html.Don't use paragraph tag.Skip html tag. Instead of em tag use u tag. Return only html, nothing else. ### Markdown text: ${question}`,
        },
      ],
